            ...
            if conn:
                ...
                if attendance:
                    for each in attendance:
                        ...
                        atten_date = fields.Date.to_string(atten_date)
                        if user:
                            for uid in user:
                                if uid.user_id == each.user_id:
                                    ...
                                    if get_user_id:
                                        ...
                                        check_var = att_obj.search([('employee_id', '=', get_user_id.id), ('check_in', '<=', atten_time), ('check_out', '>=', atten_time)])
                                            
                                            
                                        if not att_var and not check_var:
                                            att_obj.create({'employee_id': get_user_id.id,'check_in': atten_time})
                                            
                                        elif len(att_var) == 1 and not check_var and datetime.strptime(atten_time, '%Y-%m-%d %H:%M:%S') > att_var.check_in:
                                            attendance_time = datetime.strptime(atten_time, '%Y-%m-%d %H:%M:%S')
                                            attendance_date = attendance_time.date()
                                            if attendance_date == att_var.check_in.date():
                                               att_var.write({'check_out': atten_time})
                                            else:
                                               att_var.write({'check_out': att_var.check_in + timedelta(hours=7), 'note': 'Erorr in Check out/No checkout in the specific day.'})
                                               att_obj.create({'employee_id': get_user_id.id,'check_in': atten_time})
                                        else:
                                            if att_var and datetime.strptime(atten_time, '%Y-%m-%d %H:%M:%S') > att_var[-1].check_in:
